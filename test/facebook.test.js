...
describe("FACEBOOK TESTING", function() {
  ...
  describe("fb.getPostInfo", function() {
    ...
    it("Should return an object with givenInput, username, id, post_id, fan_count, shares, comments, and likes", function(done) {
      let answer = fb.getPostInfo(userFull);
      expect(answer).to.eventually.be.an('object');
      expect(answer).to.eventually.have.all.keys('username', 'givenInput', "id", 'fan_count', 'post_id', 'shares', 'comments', 'likes').notify(done);
  ...
  describe("fb.facebook", function() {
    ...
    it("Should do exactly what we want it to (case 4)", function(done) {
      ...
      expect(answer).to.eventually.be.an('object');
      expect(answer).to.eventually.have.all.keys('username', 'givenInput', "id", 'fan_count').notify(done);
    })
    ...
    it("Should throw an error when stuff breaks", function(done) {
      ...
      expect(answer).to.eventually.be.an('object');
      expect(answer).to.eventually.have.all.keys('username', 'givenInput', 'error').notify(done);
    })
